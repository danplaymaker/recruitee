...
// when a field's API slug drifts (e.g. "department" becomes "department-2").
...
function buildFieldData(offer, { slugs, brand }) {
  const data = {};
  ...
  if (slugs.department && offer.department) data[slugs.department] = offer.department;
  if (slugs.brand && brand) data[slugs.brand] = brand;
  return data;
}
...
export async function syncJobs({ collectionId, brand = '', logger = console } = {}) {
  ...
  for (const offer of offers) {
    ...
    seenSlugs.add(slug);
    const fieldData = buildFieldData(offer, { slugs, brand });
